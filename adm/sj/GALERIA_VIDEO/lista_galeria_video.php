...
?>
  <div class="panel panel-default">
    <div class="panel-heading"> Galerias de Vídeos </div>
    ...
    <div class="table-responsive">
      <table class="table table-striped">
        ...
        <?php
          ...
          $sql = "SELECT gp.id as id_grupo, gp.nome as grupo, tp.id as id_topico, tp.nome as topico, gl.id, gl.titulo, gl.ativo, gl.compartilhado, gl.home, DATE_FORMAT(gl.cadastro, '%d/%m/%Y - %H:%i') AS data FROM grupos gp INNER JOIN topicos tp ON gp.id = tp.id_grupo INNER JOIN galerias_video gl ON tp.id = gl.id_topico $w";
          $query = mysql_query("${sql} ORDER BY ${ordem} LIMIT ${start}, ${qnt}") or die(mysql_error());
          if(mysql_num_rows($query)):
            while($ln = mysql_fetch_array($query)):
              $res1  = mysql_query("SELECT count(id) as count FROM videos WHERE galerias_id = ".$ln['id']);
        ...
        ?>
              <tr>
                ...
                <td><?=$ln['data'];?></td>
                <td><?=$total;?> vídeo(s)</td>
                <td><a href="?op=carrega_video&grupo=<?=$ln['id_grupo'];?>&topico=<?=$ln['id_topico'];?>&galeria=<?=$ln['id'];?>" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-plus"></span></a></td>
                <td>
                  <a href="index.php?op=altera_galeria_video&op1=<?=$ln['id'];?>" class="btn btn-warning btn-xs"><span class="glyphicon glyphicon-pencil"></span></a>
                  <a href="index.php?op=delete_galeria_video&op1=<?=$ln['id']?>" class="btn btn-danger btn-xs" onclick="return confirm( 'Deseja realmente excluir esta galeria  \n (<?=$ln['titulo'];?>)? ' );"><span class="glyphicon glyphicon-remove"></span></a>
                  <a href="?op=clonar_galeria_video&op1=<?=$ln['id']?>" class="btn btn-primary btn-xs" title="Clonar Galeria de Foto">
                ...
                <td>
                  <?php
                    switch($ln['ativo']):
                      case 'S':
                        echo '<img src="img/aprovado.png"  alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="at" data-id="'.$ln['id'].'" data-ac="N" data-tp="1"/>';
                        break;
                      case 'N':
                        echo '<img src="img/bloqueado.png" alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="at" data-id="'.$ln['id'].'" data-ac="S" data-tp="1"/>';
                        break;
                    endswitch;
                  ?>
                </td>
                <td>
                  <?php
                    switch($ln['compartilhado']):
                      case 'S':
                        echo '<img src="img/aprovado.png"  alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="cp" data-id="'.$ln['id'].'" data-ac="N" data-tp="2"/>';
                        break;
                      case 'N':
                        echo '<img src="img/bloqueado.png" alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="cp" data-id="'.$ln['id'].'" data-ac="S" data-tp="2"/>';
                        break;
                    endswitch;
                  ?>
                </td>
                <td>
                  <?php
                    switch($ln['home']):
                      case 'S':
                        echo '<img src="img/aprovado.png"  alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="hm" data-id="'.$ln['id'].'" data-ac="N" data-tp="2"/>';
                        break;
                      case 'N':
                        echo '<img src="img/bloqueado.png" alt="" style="margin-bottom: -3px;" name="ativar" data-url="GALERIA_VIDEO/acao.php" data-tipo="hm" data-id="'.$ln['id'].'" data-ac="S" data-tp="2"/>';
                        break;
                    endswitch;
                  ?>
                </td>
