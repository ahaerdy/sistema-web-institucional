...
<div class="page-header title-article">
  <h2>Mensagens | <small>Caixa de entrada</small></h2>
</div>
<div class="row">
        ...
        <tbody>
          <?php
            $sql = "SELECT m.*, DATE_FORMAT(m.criado_em, '%d/%m/%Y ás %H:%i') as data, u1.nome as remetente_nome, u2.nome as destinatario_nome, c.categoria
                    ...
                    WHERE m.enviado_por_id != ".getIdUsuario()." AND
                          m.destinatario_id = ".getIdUsuario()." AND
                          m.visivel_destinatario = 'S'";
          ...
          ?>
                <tr class="link link-message" data-id="<?=$ln['id']; ?>" <?=($ln['visualizado']=='N')?'class="tr-nao-visualizado active"':'';?>>
                  ...
                  <td nowrap="nowrap">
                    <div class="btn-group">
                      ...
                      <a href="index.php?op=compor_mensagem&mensagem=<?=$ln['id'];?>" class="btn btn-primary btn-sm">responder</a>
                      <a href="index.php?op=remover_mensagem&mensagem=<?=$ln['id'];?>" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-remove"></span></a>
                    </div>
                  </td>
                </tr>
          <?php
