<?php 
  ...
  $idUsuario = (int)getParam('usuario_id');
  if(!$idUsuario):
  ...
  // header("Content-Disposition: attachment; filename=relatorio_atividades".date("Y-m-d-H-i-s").".xls");
  // header("Pragma: no-cache");

  $month = (getParam('mes'))? "MONTH(l.criado_em) = ".(int)getParam('mes'):"YEAR(l.criado_em) = ".date('Y');
  ...
  $sql = mysql_query("SELECT u.nome as usuario, 
                      ...
                      WHERE $month AND l.usuario_id = ".$idUsuario) or die(mysql_error());
  ...
  <table border="1" cellpadding="5" cellspacing="5">
    <tr>
      <td colspan="9">
        <h1>Relatório de Atividades &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <?=date("d/m/Y H:i"); ?></h1>
        <p>Usuário: <?= $row['usuario']; ?></p>
        <p>Mês: <?= $row['mes']; ?></p>
        <p>Ano: <?= $row['ano']; ?></p>
      </td>
    </tr>
