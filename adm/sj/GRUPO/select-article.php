<div class="col-lg-4">
  <div class="panel panel-default" id="dv-group1">
    <div class="panel-heading">ARTIGOS, FOTOS, VÍDEOS, LIVROS</div>
    <div class="panel-body">
      <div class="input-group">
        <?
          mysql_data_seek($resGrupo,0);
          $rows = getDataFormSelect($resGrupo);
          echo $form->select('grupos_cadastrados_art[]', '', $rows, array('class'=>'form-control'), '-');
        ?>
      ...
      <?php
        if(!empty($id_grupo)):
          $res = mysql_query("SELECT * FROM grupos_dependente WHERE grupos_id = ${id_grupo} AND tipo = 'artigo'");
          while ($ln = mysql_fetch_assoc($res)):
      ?>
            <div class="group">
              <div class="input-group">
                <select name='grupos_cadastrados_art[]' class="form-control" id="group1">
                  <option value=""></option>
                  <?php
