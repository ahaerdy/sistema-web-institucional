<?php
  ...
  if(isPost()):
    ...
    $id_topico_pai  = (int) $_POST['id_topico_pai'];
    $grupo          = (int) $_POST['grupo'];
    $titulo         = addslashes($_POST['titulo']);
    $data           = explode('/', $_POST['data']);
    $cadastro       = $data[2] . '-' . $data[1] . '-' . $data[0] . ' ' . $_POST['hora'];
    if(!empty($titulo) && !empty($id_topico_pai) && !empty($grupo)):
      mysql_query("INSERT INTO topicos (id_topico, id_grupo, nome, id_usuario, cadastro) values ('$id_topico_pai','$grupo','$titulo','$id_usuario','$cadastro')");
        ...
        if (mysql_num_rows($res)):
          ...
          if(file_exists($dir)):
            ...
            if($dir0 && $dir1 && $dir2 && $dir3 && $dir4 && $dir5)
              setFlashMessage('Tópico cadastrado com sucesso!', 'success');
            else
              setFlashMessage('Tópico cadastrado com sucesso, porem não foi possível criar os diretório do tópico, comunique o administrador!', 'warning');
          else:
            setFlashMessage('Tópico cadastrado com sucesso, porem não foi possível criar os diretório do grupo, comunique o administrador!', 'warning');
          endif;
        else:
          setFlashMessage('Tópico cadastrado com sucesso, porem não foi possível criar os diretório do grupo, comunique o administrador!', 'warning');
        endif;
      else:
        setFlashMessage('Não foi possivel criar o tópico, comunique o administrador', 'danger');
      endif;
    else:
      setFlashMessage('Informe o título ou grupo para criar o tópico', 'warning');
    endif;
