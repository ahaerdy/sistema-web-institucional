<?
  ...
  $orderParam = getOrderByParamUrl('at_ordem_topico', 'at.titulo');
  $ordem_topicoParametro = $orderParam['order'];
  $ordemSQL[] = $orderParam['fieldSql'];
  $orderParam = getOrderByParamUrl('at_ordem_data', 'at.cadastro');
  ...
  $ordem = getOrder($ordemSQL, 'at.cadastro DESC, tp.nome ASC, at.titulo ASC');

  $sql = "SELECT
            CONCAT('/arquivo_site/',gp.diretorio,'/topicos/',tp.diretorio,'/artigos/',at.diretorio,'/',at.capa) as foto,
            at.capa,
            tp.nome as assunto, 
            at.id, 
            at.titulo, 
...
?>
    <div id="1" class="tab-pane active">
      <div class="well well-sm grid-toolbar">
        <div class="row">
          <div class="col-xs-4 col-sm-6 col-md-8 col-lg-10">
            <div class="btn-group">
              <a href="?<?="${urlGrid}&tipo_list=B&aba=1";?>" class="btn btn-success <?=getActiveNav($tipoList,'B',0);?>"><span class="glyphicon glyphicon-th"></span></a></li>
              <a href="?<?="${urlGrid}&tipo_list=T&aba=1";?>" class="btn btn-success <?=getActiveNav($tipoList,'T',0);?>"><span class="glyphicon glyphicon-align-justify"></span></a></li>
              <a href="?<?="${urlGrid}&tipo_list=L&aba=1";?>" class="btn btn-success <?=getActiveNav($tipoList,'L',0);?>"><span class="glyphicon glyphicon-list"></span></a></li>
            </div>
          </div>
          <div class="col-xs-8 col-sm-6 col-md-4 col-lg-2">
            <div class="btn-group pull-right">
              ...
              <ul class="dropdown-menu" role="menu">
                <li><a href="?<?="${urlGrid}&aba=1&at_ordem_topico=${ordem_topicoParametro}";?>">Título <span class="glyphicon <?=($ordem_topicoParametro =='A')?'glyphicon-sort-by-alphabet':'glyphicon-sort-by-alphabet-alt';?>"></span></a></li>
                <li><a href="?<?="${urlGrid}&aba=1&at_ordem_data=${ordem_dataParametro}";?>">Data <span class="glyphicon <?=($ordem_dataParametro =='A')?'glyphicon-sort-by-attributes-alt':'glyphicon-sort-by-attributes';?>"></span></a></li>
              </ul>
            </div>
          </div>
      ...
      <div class="row">
        <? $urlLink = '?op=mostra_artigo&artigo='; ?>
        <? $typeColor = 'success'; ?>
